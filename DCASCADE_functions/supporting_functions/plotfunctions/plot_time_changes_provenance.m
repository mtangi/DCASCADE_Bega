...
ID_prov = p.Results.ID_prov; %id of the provenance data used as baseline
...
ID1stData = p.Results.ID1stData;

ID1stData_tot = ID1stData(1);
ID1stData_prov =  ID1stData(2);
...
if isempty(colorplot)
    colorplot = cMap([ID1stData_tot,ID1stData_prov,ID2ndData],:);
end
...
if equal_axis 
    yinterval1 = [ min( data_plot{ID1stData_tot,2}(:,reaches ) ,[], 'all') ; max(data_plot{ID1stData_tot,2}(:,reaches ) ,[], 'all')] ; 
...
for i=1:length(reaches)
    ...
    area(data_plot{ID1stData_tot,2}(:,reaches(i) ) , 'FaceColor', colorplot(1,:) , 'LineWidth' , linewidth)
    hold on
    area(data_plot{ID1stData_prov,2}{ID_prov}(:,reaches(i) ) , 'FaceColor', colorplot(2,:) , 'LineWidth' , linewidth)
