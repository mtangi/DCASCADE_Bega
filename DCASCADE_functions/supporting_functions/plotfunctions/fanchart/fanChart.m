...
cCenter = results.cCenter;
lWidth = results.LineWidth;
...
if ~isempty(cCenter)
    lh = line(xvals, centerline, 'LineWidth', lWidth, 'Color', cCenter, 'Parent', parent);
end
...
HSV = rgb2hsv(color);
shades = log(linspace(exp(.2),exp(1),ncolors));
...
function map = shadesOfColor_alt(color, ncolors)
fade_color = rgb2hsv(color);
fade_color(2) = 0.1;
fade_color(3) = 1;
fade_color = hsv2rgb(fade_color);
%fade_color = 1 - color.*0.95;
map = [linspace(fade_color(1),color(1),ncolors)', linspace(fade_color(2),color(2),ncolors)', linspace(fade_color(3),color(3),ncolors)'];
